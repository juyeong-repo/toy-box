...
export class TasksService {
  ...
  /**
   * 태스크 검색
   * - 제목 또는 작성자 이름에 검색어가 포함된 태스크를 조회
  ...
  /** 태스크 생성 - 현재 로그인한 사용자가 작성자로 자동 설정 */
  ...
  /**
   * 태스크 수정 - 낙관적 잠금을 통한 동시성 제어
   *
   * [동시성 문제 정의]
   * 두 명의 사용자가 동시에 같은 태스크를 수정하는 경우:
   * 1. 사용자 A가 태스크를 조회 (version: 1)
   * 2. 사용자 B가 같은 태스크를 조회 (version: 1)
  ...
  async update(id: string, dto: UpdateTaskDto): Promise<Task> {
    ...
    try {
      // 클라이언트가 보낸 version과 DB의 version을 비교
      // 불일치 시 OptimisticLockError 발생
      await this.em.lock(task, LockMode.OPTIMISTIC, dto.version);
      ...
      // flush 시점에도 DB의 version과 엔티티의 version을 비교하므로
      // lock과 flush 사이에 다른 요청이 version을 변경한 경우에도 충돌 감지
      await this.em.flush();
    } catch (e) {
